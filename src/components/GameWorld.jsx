...
export default function GameWorld({ onEnterCity, onEnterMatching, completedIds = [] }) {
  ...
  useEffect(() => {
    ...
    function update(dt) {
      ...
      // Near-special detection
      nearSpecialRef.current = SPECIALS.find(
        sp => Math.abs(sp.col - s.col) <= 1 &&
              Math.abs(sp.row - s.row) <= 1
      ) ?? null;
    }
    ...
    function render() {
      ...
      SPECIALS.forEach(sp => {
        ...
        drawLabel(ctx, sp.col, sp.row, sp.name, sp.color, false, isNear);
        if (isNear) drawPrompt(ctx, sp.col, sp.row, sp.name);
      });
